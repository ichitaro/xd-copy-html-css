...
| `--relative-length` | `--rl` | なし &#124; `fr` | 主軸方向のサイズを%指定にする。<br>割合は XD 上のサイズに準ずる。<br>コンテナの主軸方向のサイズは固定、<br>パディングは強制的に 0 になる。<br>値に `fr` を指定すると flex 係数になる。 |
| `--relative-width` | `--rw` | なし | 対称軸方向のサイズを%指定にする。<br>割合は XD 上のサイズに準ずる。<br>コンテナの対称軸方向のサイズは固定、<br>パディングは強制的に 0 になる。 |
